...
(defprod element (obj ((name symbol)
  ...
  (to-html (path) (multitags 
  		       ...
  		       (a (list :href (concatenate 'string "#" (synth to-string (synth to-brief schema) 0)))
                          (synth to-brief schema))
                       ;; (p nil (synth to-brief schema))
                       (synth to-html element path)
...
(defprod element (replica ((name symbol)
                           (schema jsschema)
                           (element element)))
  (to-list () `(replica (:name ,name 
                               :schema ,(synth to-list schema) 
                               :element ,(synth to-list element))))
  (to-html (path) (multitags 
                   (text "Replica dinamica identificata con ~a collegata al seguente formato dati:" (lower-camel name)) 
                   (p nil (synth to-brief schema)) 
                   (synth to-html element path)
                   (p nil (text "Essa produce il seguente oggetto JSON:")
                      (code nil (synth to-string (synth to-model (replica name schema element)))))))
  (to-brief (path) (synth to-html (replica name schema element) path))
...
(defprod element (arr ((name symbol)
  ...
  (to-list () `(arr (:name ,name 
                           ...
                           :element ,(synth to-list element))))
  (to-html (path) (multitags 
                   (text "Sezione dinamica identificata con ~a collegata al seguente formato dati:" (lower-camel name)) 
                   (p nil (synth to-brief schema)) 
                   (synth to-html element path)
                   (p nil (text "Essa produce il seguente oggetto JSON:")
                      (code nil (synth to-string (synth to-model (arr name schema element)))))))
  (to-brief (path) (synth to-html (arr name schema element) path))
  (to-model () (jarray (synth to-model element)))
  (toplevel () nil)
  (req (path) nil)
  (template (&optional father) (div (list :class "form-group")
