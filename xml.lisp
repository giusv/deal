...
;;      (labels ((open-tag (as) (format nil "<~(~a~)~{ ~(~a~)=\"~a\"~}>" ,(lower-camel name) as))
...
(defprod xml (node ((name string) &optional (attrs attribute) &rest (nodes node)))
  (to-doc () (labels ((open-tag (as) (format nil "<~a~{ ~(~a~)=\"~(~a~)\"~}>" (lower-camel name) as))
		      (close-tag () (format nil "</~a>" (lower-camel name)))
		      (open-close-tag (as) (format nil "<~a~{ ~(~a~)=\"~(~a~)\"~}/>" (lower-camel name) as)))
	       (if (null nodes)
		   (text  (open-close-tag attrs))
		   (vcat (text  (open-tag attrs))
